...
def _facts(case: dict) -> tuple[dict, list[dict]]:
    if case["input_kind"] == "document":
        return ({"chief_complaint": None, "summary": "Outside ketorolac prescription",
    ...
    if case["input_kind"] == "text":
        return ({"chief_complaint": None, "summary": "Cancel EKG", "medications": [],
                 "follow_ups": [], "emotional_tone": None},
                [{"ntype": "lab_order", "label": "EKG", "category": None,
                  "polarity": "denied", "confidence": 1.0, "detail": "cancel"}])
    return ({"chief_complaint": "Chest pain", "summary": "Chest pain improved to 2/10.",
...
def _language_json(prompt: str, **_kwargs):
    if "billing codes" in prompt:
        return {"codes": [{"system": "ICD-10", "code": "I48.91", "evidence": "Atrial fibrillation"},
                          {"system": "CPT", "code": "99232", "evidence": "moderate complexity"}]}
    return {"priority_note": "Review active Guardian alerts", "situation": "Chest pain improved.",
...
def evaluate_agent_case(case: dict) -> dict:
    with temp_db():
        ...
        llm.ask_tools = lambda *args, **kwargs: (_ for _ in ()).throw(RuntimeError("repeatable fallback"))
        graph.structure_and_extract = lambda _text: _facts(case)
        llm.ask_json, llm.ask = _language_json, lambda *args, **kwargs: "Resumen claro para la paciente."
        try:
            source = agent.source_kind_for(case["input_kind"], case["text"])
            bundle = agent.run_agent(agent.ToolContext(patient["id"], None, source, "es"),
                                     case["text"], case["input_kind"])
        finally:
        ...
        return {"id": case["id"], "passed": route["passed"] and
                (not case.get("expected_critical_alert") or critical),
...
def run() -> dict:
    ...
    return {"agent": agent_results, "coding": coding_results,
