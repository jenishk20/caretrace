#!/usr/bin/env bash
# MedSignal demo launcher — starts Ollama with speed-ups, then the app.
#
...
sleep 1
ollama serve >/tmp/ollama-medsignal.log 2>&1 &
sleep 2
...
echo "→ starting MedSignal on http://localhost:8000"
